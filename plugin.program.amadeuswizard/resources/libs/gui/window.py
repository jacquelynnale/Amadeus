...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = "[COLOR {0}]ברוכים הבאים ל- {1}[/COLOR]\n\n[COLOR {2}]לחץ על [B]התקנת בילד [/B]להתחלת ההתקנה.[/COLOR]".format(CONFIG.COLOR2, CONFIG.ADDONTITLE, CONFIG.COLOR2)
...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fanart = kwargs['fanart']
            #self.msgupdate = "Update available for installed build:\n[COLOR {0}]{1}[/COLOR]\n\nCurrent Version: v[COLOR {2}]{3}[/COLOR]\nLatest Version: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Recommened: Fresh install[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
            self.msgupdate = "גרסה חדשה זמינה לבילד:\n[COLOR {0}]{1}[/COLOR]\n\nגרסת בילד נוכחית: v[COLOR {2}]{3}[/COLOR]\nגרסת בילד עדכנית: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*מומלץ לעדכן! התעדכנו בשינויים בקבוצת הטלגרם.[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
    ...
    latest_version = 'גרסת בילד עדכנית: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
    ...
    install = xbmcgui.Dialog().yesno(CONFIG.ADDONTITLE, final_msg,
                                     yeslabel='לחץ לעדכון גרסה', nolabel='הזכר מאוחר יותר')
    if install:
